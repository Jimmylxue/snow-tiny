...
function main(input: string, output: string): TFileObject {
	...
	allFilesSpinner.succeed(
		`共扫描到 ${chalk.blueBright(uploadList.length)} 个文件`
	)
	// 分配任务压缩&下载图片
		...
		work.on('message', (details: TDetail[]) => {
			...
			details.forEach((item: TDetail) => {
				...
				progress.render({
					...
					token: `共${chalk.blueBright(uploadList.length)}文件 ${chalk.green(
						succeedNum
					)} 个成功  ${chalk.red(failNum)} 个失败`,
				})
			})
			pageNum--
